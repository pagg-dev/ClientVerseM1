...
export default class ConditionBuilder extends LightningElement {
    ...
    operations = [
        {
            label: 'equals',
            value: ' = ',
            types: 'String,Picklist,Url,Email,TextArea,Reference,Phone,Date,DateTime,Currency,Double,Boolean,Int,Address,NestedField,Child'
        },
        {
            label: 'not equals',
            value: ' != ',
        ...
        conditions.forEach(curCondition => {
            for (let i = 0; i < this.operations.length; i++) {
                ...
                if (curCondition.indexOf(curLogicType.value) !== -1) {
                    ...
                    let fieldDescriptor = this.getFieldDescriptor(conditionParts[0].trim());
    ...
    handleConditionChanged(event) {
        ...
        if (changedCondition) {
            ...
            if (fieldDescriptor) {
                ...
                changedCondition.dataType = this.getDataType(fieldDescriptor);
            } else {
                if (this._fields) {
                    this._fields.push({
            ...
            if (changedCondition.operation == 'Child') {
                changedCondition.childRelationship = newCondition.childRelationship;
                changedCondition.childField = newCondition.childField;
            }
            this.dispatchConditionsChangedEvent();
        }
    }
    ...
    constructWhereClause() {
        ...
        this._conditions.forEach(c => {
            ...
            const built = this.buildCondition(c);
            console.log('built');
            console.log(JSON.stringify(built));
            if (built.type === 'parent') {
                parentConditions.push(built.condition);
            }
            if (built.type === 'child') {
                if (!childConditions[built.relationshipName]) {
                    childConditions[built.relationshipName] = [];
                }
                childConditions[built.relationshipName].push(built.condition);
            }
        });

        let parentWhere = "";
        if (parentConditions.length) {
            parentWhere = parentConditions.join(` ${this.logicType} `);
        }

        let childWhereStrings = [];

        for (let rel in childConditions) {
            const subconditions = childConditions[rel];

            const combined = subconditions.join(" AND ");

            childWhereStrings.push(combined);
        }

        let finalWhere = "";

        if (parentWhere && childWhereStrings.length) {
            finalWhere = parentWhere + " AND " + childWhereStrings.join(" AND ");
        }
        // else if (parentWhere) {
        //     finalWhere = parentWhere;
        // }
        else if (childWhereStrings.length) {
            finalWhere = childWhereStrings.join(" AND ");
        }
        console.log("finalWhere: " + finalWhere)
        return {
            parentWhere: parentWhere,
            childWhere: childConditions,
            fullWhere: finalWhere
        };
    }
    ...
    buildCondition(condition) {
        ...
        const value = this.getConditionValue(condition);
        const op = condition.operation;

        // CHILD LOGIC
        if (op === 'Child') {
            const relationshipName = condition.childRelationship;   
            const childObject = condition.childObject;             
            const childField = condition.childField;
            const parentIdField = 'Id';                             

            // Stop undefined SOQL early
            if (!relationshipName || !childField) {
                return {
                    type: 'invalid',
                    reason: 'Missing childRelationship or childField'
                };
            }
    ...
    isChildField(fieldName) {
        return fieldName && fieldName.includes('.');
    }

    getFieldDescriptor(fieldName) {
        // direct match first
        let direct = this._fields.find(f => f.value === fieldName);
        if (direct) return direct;
        // child field → split by dot
        if (this.isChildField(fieldName)) {
            let parts = fieldName.split('.');
            let baseField = parts[parts.length - 1];
            let directMatch = this._fields.find(f => f.value === baseField);

            if (directMatch) {
                // clone and modify descriptor
                return {
                    ...directMatch,
                    dataType: directMatch.dataType,
                    isChild: true,
        ...
        // fallback
        return { value: fieldName, dataType: 'NestedField', isChild: true };
    }

    getDataType(fieldDescriptor) {
        if (fieldDescriptor.isChild) return 'Child';
        return fieldDescriptor.dataType;
    }
}