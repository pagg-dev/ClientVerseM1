...
const buildOptions = (data) => {
    ...
    Object.keys(data.fields).forEach(api => {
        let f = data.fields[api];
        let isReference = f.dataType === "Reference";
        let relatedObject = f.referenceToInfos?.length ? f.referenceToInfos[0].apiName : null;

        tempOptions.push({
            type: isReference ? "SObject" : f.dataType,
            label: f.label || f.apiName,
            value: f.apiName,
            isCollection: false,
            objectType: relatedObject,
            optionIcon: iconsPerType[isReference ? "SObject" : f.dataType],
            isSObject: isReference,
            relationshipName: f.relationshipName,
            displayType: isReference ? relatedObject : f.dataType,
            key: "field_" + localKey++
        });
    });

    // ------- CHILD RELATIONSHIPS (THIS WAS BROKEN) -------
    if (data.childRelationships) {
        data.childRelationships.forEach(rel => {
            ...
            tempOptions.push({
                type: "SObject",
                label: rel.relationshipName + " (Child Records)",
                value: rel.relationshipName,
                isCollection: true,
                objectType: rel.childObjectApiName,
                optionIcon: "utility:group",
                isSObject: true,
                relationshipName: rel.relationshipName,
                displayType: rel.childObjectApiName,
                key: "child_" + localKey++
            });
        });
    }
