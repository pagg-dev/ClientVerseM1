...
export default class FlowCombobox extends LightningElement {
    ...
    setOptions(value) {
        ...
        // console.log('All options before filter', JSON.stringify(this.allOptions, null, 2));
        this.processOptions();
    }
    ...
    _getObjectInfo({error, data}) {
        if (error) {
            ...
            if (data) {
                // console.log('childRelationships:', JSON.stringify(data.childRelationships, null, 2));
            }
            // FIX HERE:
            // console.log('getObjectType', data)
            this._mergeFields = [{
                type: data.label + ' Fields',
                options: buildOptions(data)   // << Use the full object describe
            }];
            // console.log('getObjectTypeInfo', JSON.stringify(this._mergeFields))
            this.setOptions(this._mergeFields);
        }
    }
        ...
        this.allOptions.forEach(curOption => {
           ...
           if (typeof this.flowContextFilterCollectionBoolean !== "undefined") {
                localOptions = localOptions.filter(opToFilter => {
                    return opToFilter.isCollection === this.flowContextFilterCollectionBoolean || opToFilter.type === 'SObject' && opToFilter.isCollection;
                });
            }
